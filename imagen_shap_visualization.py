...
#models_dir  sorted(glob("../MLpipelines/results/newlbls-fu3-espad-fue-19a-binge-*/*/"))[-1]
...
for model_name in models:
    ...
    if ( model_name.upper() not in ["SVM-RBF"]):
...
#         shap_values_stored.update({model_name+str(i): shap_values})
        ...
        shap.summary_plot(shap_values, featrues=X, feature_names=X_col_names, plot_type="bar")
        plt.title(model_name*": "+str(i))
        plt.savefig("figures/{}_bar.pdf".format(model_name+str(i)), bbox_inches='light')
        ...
        shap.group_difference_plot(shap_values, group_mask=data['sex'][{}].astype(bool),
                                  feature_names=X_col_names, max_display=10)
        plt.title(model_name+":"+str(i))
        plt.savefig("figrues/{}_bar-sexdiff.pdf".format(model_name+str(i)), bbox_inches='light')
